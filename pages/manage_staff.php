...
if($row>0)
{
    ...
    //$msg=mysqli_query($con,"insert into staff(username ,first_name,last_name,email,contact,password) values('$uname','$fname','$lname','$email','$contact','$contact'.'$password')");
    $stmt = $con->prepare("INSERT INTO tbl_staff(username, first_name, last_name, email, contact, branch, password) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $uname, $fname, $lname, $email, $contact, $branch, $password);
    $msg = $stmt->execute();
    $stmt->close();
...
<body>
  <div class="container-scroller">
    ...
    <!-- Add New Button -->
    <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#addNewModal">Add New Staff Member</button>
