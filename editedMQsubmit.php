    ...
    foreach ($cover_premium_pairs as $pair) {
        ...
        for ($i = 0; $i < max(count($cover_codes), count($cover_amts), count($cover_premiums), count($cover_calseqs), count($cover_areas)); $i++) {
            ...
            if (!empty($cover_code) || !empty($cover_amt) || !empty($cover_premium) || !empty($cover_calseq) || !empty($cover_area)) {
                ...
                //echo "Cover Code: $cover_code, Cover Amount: $cover_amt, Cover Premium: $cover_premium, Cover calseq: $cover_calseq, Cover Area: $cover_area<br>";

                // Ensure the cover_premium is correctly formatted as a float
...
$sql_mt = "INSERT INTO rev_quotation_mt (old_quote_no, new_quote_no, comp_codce, prod_code, class_code, cus_name, cus_address, cus_mobile, cus_email, vehi_number, vehi_reg_status, vehi_make_model, vehi_seating_capacity, vehi_year_of_manu, vehi_fuel_type, vehi_usage, sum_ins, ncb_rate, ncb_amt, mr_rate, mr_amt, vehi_rate, basic_rate, date_period, basic_premium, tot_premium, vat_amt, pol_fee, ssl_amt, admin_fee, prnt_status, cal_date, user_ID, user_brnch) 
           VALUES ('$quote_no','$NEWquote_no', '$compCode','$prodcode','NULL','$clientName','$clientAddress','$mobile_number','$email_address','$vehiregNo','$registrationStatus','$makeModel','$seatingCapacity','$manufactureYear','$fuelType','$vehiusage','$sum_ins','$ncbRate','$ncb_MRamt','$mrRate','$ncb_MRamt','$rate','$basicrate','$datePeriod', '$basic_premium', '$totalPremium','$VATChargers','0','0','$adminChargers','$printStatus','$calDate','$B_username','$branch')";
