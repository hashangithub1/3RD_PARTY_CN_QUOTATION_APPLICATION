...
$page_name = !empty($_SESSION['page_name']) ? $_SESSION['page_name'] : null;

// Check Pass
// Check if the engine number already exists in the database
$checkqotationSQL = "SELECT quote_no FROM quotation_mt WHERE quote_no = '$quote_no'";
$resultQuotation = $con->query($checkqotationSQL);

if ($resultQuotation->num_rows > 0) {
    echo "<script>alert('Quotation number already exists.');</script>";
    echo "<script type='text/javascript'> document.location = '$page_name'; </script>";
    exit();
}
// End
...
$basic_premium = !empty($_SESSION['basic_premium']) ? $_SESSION['basic_premium'] : null;
...
$comp_excesses       = !empty($_SESSION['comp_excesses']) ? $_SESSION['comp_excesses'] : 0;
$Age_Exces      = !empty($_SESSION['Age_Exces']) ? $_SESSION['Age_Exces'] : 0;
$discount_amt = !empty($_SESSION['amount']) ? $_SESSION['amount'] : 0;
    ...
    foreach ($cover_premium_pairs as $pair) {
        ...
        for ($i = 0; $i < max(count($cover_codes), count($cover_amts), count($cover_premiums), count($cover_calseqs), count($cover_areas)); $i++) {
            ...
            if (!empty($cover_code) || !empty($cover_amt) || !empty($cover_premium) || !empty($cover_calseq) || !empty($cover_area)) {
                ...
                $sql = "INSERT INTO quotation_cover_mt (quote_no, cover_code, cover_amt, print_flag, cal_seq, cover_premium, dis_area, cal_date) 
                        VALUES ('$quote_no', '$cover_code', '$cover_amt', '$printStatus', '$cover_calseq', '$cover_premium', '$cover_area', '$calDate')";
...
if (isset($_SESSION['FreeCoversCBT01'])) {
    ...
    $sqlFC = "INSERT INTO quotation_cover_mt (quote_no, cover_code, cover_amt, print_flag, cal_seq, cover_premium, dis_area, cal_date) 
              VALUES ('$quote_no', '$cover_code', '$coverAMT', '1', '$calSeq', '0', '$dis_area', '$calDate')";
...
$sql_mt = "INSERT INTO quotation_mt (quote_no, comp_codce, prod_code, class_code, cus_name, cus_address, cus_mobile, cus_email, vehi_number, vehi_reg_status, vehi_make_model, vehi_seating_capacity, vehi_year_of_manu, vehi_fuel_type, vehi_usage, sum_ins, ncb_rate, ncb_amt, mr_rate, mr_amt, vehi_rate, basic_rate, date_period, basic_premium, total_01, tot_premium, vat_amt, pol_fee, ssl_amt, admin_fee, comp_excesses, age_exces, discount_amt, remark, prnt_status, cal_date, user_ID, user_brnch, edit_status) 
           VALUES ('$quote_no','$compCode','$prodcode','NULL','$clientName','$clientAddress','$mobile_number','$email_address','$vehiregNo','$registrationStatus','$makeModel','$seatingCapacity','$manufactureYear','$fuelType','$vehiusage','$sum_ins','$ncbRate','$ncb_MRamt','$mrRate','$ncb_MRamt','$rate','$basicrate','$datePeriod', '$basic_premium', '$total_01_', '$totalPremium','$VATChargers','0','0','$adminChargers', '$comp_excesses','$Age_Exces', '$discount_amt', '$remark','$printStatus','$calDate','$B_username','$branch', '1')";
...
$usage = !empty($_SESSION['usage']) ? ucfirst($_SESSION['usage']) : null;
...
class PDF extends FPDF
{
    ...
    function _Arc($x1, $y1, $x2, $y2, $x3, $y3)
    {
     ...
     function Footer()
     {
         ...
         $this->Cell(0, 5, 'Issued By: '.$GLOBALS['issuedBy'].' | Reference: '.$GLOBALS['companyProduct'].' - '.$GLOBALS['vehiType'].' | Issued Date: '.$GLOBALS['issueddate'].' | Quotation No: '.$GLOBALS['quote_no'], 0, 1, 'C');
